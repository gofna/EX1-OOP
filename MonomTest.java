...
public class MonomTest {
	...
	private static void test1() {
		...
		String[] monoms = {"0", "1.7x", "7x^2" , "5x", "5x^^", "kx^2", "2x^2.5"}; //should throw exception at monoms[4,5,6]
		for(int i=0;i<monoms.length;i++) {
			...
			m = new Monom(s);
			double fx = m.f(i);
			System.out.println(i+") "+m +"    \tisZero: "+m.isZero()+"\t f("+i+") = "+fx + "   \t derivative: " + m.derivative());

			}
			catch(Exception e){
				e.printStackTrace();
				System.err.println(i +")  "+ e + "-  " +  monoms[i]);
				
			}
		}
		Monom m1 = new Monom(monoms[1]);
		Monom m2 = new Monom(monoms[3]);
		System.out.println("m1:  " +m1 );
		System.out.println("m2:  " +m2 );
		m1.add(m2);
		System.out.println("after adding m2 to m1 : m1 = " + m1);
		m2.multiply(m1); // m1 = 8x
		System.out.println("after multiply m2 by m1 :  m2 = " + m2 + "\n");
